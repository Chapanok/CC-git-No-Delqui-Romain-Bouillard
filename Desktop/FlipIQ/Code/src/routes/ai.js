...
// Auth middleware (optionnel, fallback si non configuré)
let maybeAuthScan = (req, _res, next) => next();
try {
  const auth = require('../middleware/auth');
  if (auth && typeof auth.maybeAuthScan === 'function') maybeAuthScan = auth.maybeAuthScan;
} catch { /* ignore */ }
...
// POST /api/ai/scan
// -------------------------------------
router.post('/scan', maybeAuthScan, upload.array('images', 5), async (req, res) => {
...
// -------------------------------------
router.post('/describe', maybeAuthScan, express.json(), async (req, res) => {
  try {
    const {
...
// C'est LA route qui vous manquait !
// -------------------------------------
router.post('/listing', maybeAuthScan, express.json(), async (req, res) => {
    try {
      const {
        title = '', condition = 'bon état', options = {}, color = null,
        priceHint = null, currency = 'EUR', specs = null, lang = 'fr', hints = {},
